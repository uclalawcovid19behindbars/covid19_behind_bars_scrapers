...
top_three_by_state <- percentage_change %>%
    group_by(State) %>%
...
top_five_states <- top_three_by_state %>%
    group_by(State) %>%
...
# make plots
plot_bars_by_state <- function(dat, xvar, yvar) {
...
plots <- top_three_by_state %>%
    filter(State %in% top_five_states) %>%
    plot_bars_by_state(., xvar = "Name", yvar = "ten_day_change")
plots$plot
```
