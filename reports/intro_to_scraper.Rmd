...
rep_count <- slist$report_df %>%
    select(
        ...
        `Staff.Confirmed`, `Residents.Tested`, `Staff.Tested`) %>%
    pivot_longer(-State) %>%
    group_by(State) %>%
...
state_tested <- slist$current_data %>%
    filter(jurisdiction == "state") %>%
    group_by(State) %>%
    summarize(Tested = sum_na_rm(Residents.Tested), .groups = "drop")
...
map_plot <- map_df %>%
    ggplot(aes(fill = Tested, text = State)) +
    geom_sf() +
    theme_void() +
...
map_plot +
    ggtitle(
        "Incarcerated Population in State Prisons Tested",
