...
state_tested <- slist$current_data %>%
    ...
    mutate(test_rate = Tested / aug_pop,
           any_testing = ifelse(is.na(Tested), "No testing", "Some testing")) 
...
inverse_map_plot +
    ggtitle(
        "No Current COVID-19 Tests Reported for the Incarcerated Population") + 
...
## get 10-day percentage change in cases
percentage_change <- facilities %>%
    mutate(
        ten_days_ago = dplyr::lag(Residents.Confirmed, n = 10, order_by = Date),
        ten_day_change = (Residents.Confirmed - ten_days_ago) / ten_days_ago
    ) %>%
    dplyr::arrange(desc(Date)) %>%
    filter(!is.infinite(ten_day_change)) %>%    # what to do about dividing by zero?
    slice_head(n = 1) %>%
...
## TO DO: change this to be a combination of 
...
plot_bars_by_state <- function(dat, xvar, yvar, grpvar) {
  ...
  plots <- dat %>% 
    ...
    do(plot=ggplot(data=.) +
           ...
           labs(title = glue('Prison COVID Infections in {unique(.$StateFull)}'),
