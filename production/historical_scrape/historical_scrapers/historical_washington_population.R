...
# Monthly averages are saved in a single pdf file for each fiscal year 
save_local_wa_pop_data <- function(fy) {

    if (fy == 2020){
        url <- "https://www.doc.wa.gov/docs/publications/reports/400-RE002-2006.pdf"
        ...
        url <- "https://www.doc.wa.gov/docs/publications/reports/400-RE002.pdf"
    ...
    write_csv(out, stringr::str_c(
        "results/local_files/washington_population_fy", fy, ".csv"))
}
...
historical_wa_pop_restruct <- function(x, date){
    ...
    local_path <- stringr::str_c(
        "results/local_files/washington_population_fy", fy, ".csv")
    
    if (!file.exists(local_path)){
        save_local_wa_pop_data(fy)
    }
...
if(sys.nframe() == 0){
    ...
    historical_wa_pop$reset_date("DATE")
