...
florida_img_restruct <-  function(x, enhancements = seq(100, 500, by = 50)){
    base_image <- x
...
florida_img_extract <- function(x){
    x %>%
...
florida_img_scraper <- R6Class(
    ...
    public = list(
        ...
        initialize = function(
            ...
            pull_func = florida_img_pull,
            restruct_func = florida_img_restruct,
            # Rename the columns to appropriate database names
            extract_func = florida_img_extract){
            super$initialize(
...
if(sys.nframe() == 0){
    florida_img <- florida_img_scraper$new(log=FALSE)
    florida_img$raw_data
    florida_img$pull_raw()
    florida_img$raw_data
    florida_img$restruct_raw()
    florida_img$restruct_data
    florida_img$extract_from_raw()
    florida_img$extract_data
    florida_img$validate_extract()
    florida_img$save_extract()
}
